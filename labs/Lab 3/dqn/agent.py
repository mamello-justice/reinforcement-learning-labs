...
class DQNAgent:
    def __init__(
        ...
        # Q-networks
        self.Q = DQN(observation_space=observation_space,
                     action_space=action_space).to(self.device)
        self.Q_target = DQN(observation_space=observation_space,
                            action_space=action_space).to(self.device)
    ...
    def optimise_td_loss(self):
        ...
        with torch.no_grad():  # Not used in gradient calculation
            values_target = self.Q_target(next_states).max(
                dim=1, keepdim=True)[0].flatten()
            target = rewards + self.gamma * values_target * (1 - dones)

        prediction = self.Q(states).gather(
            dim=1, index=actions.reshape((-1, 1))).flatten()
    ...
    def load(self, path):
        ...
        self.replay_buffer._next_idx = len(
            self.replay_buffer._storage) % self.replay_buffer._maxsize
